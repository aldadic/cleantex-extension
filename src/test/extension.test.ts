...
suite('Extension Test Suite', () => {
	...
	test('scanMacros', () => {
		...
		`;
		let scanned = scanMacros(input, logger);
		assert.strictEqual(Object.keys(scanned.macros).length, 3);
		assert.strictEqual(scanned.macros['test'][0], 'test');
		assert.strictEqual(scanned.macros['test2'][0], '\\command{test}');
		assert.strictEqual(scanned.macros['revision'][0], '{\\color{red}#1}');
		assert.strictEqual(scanned.macros['test'][1], 0);
		assert.strictEqual(scanned.macros['test2'][1], 0);
		assert.strictEqual(scanned.macros['revision'][1], 1);
	});
